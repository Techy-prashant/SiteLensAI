...
class HazardClassifier:
    ...
    def classify(self, vlm_output: dict[str, Any]) -> ClassificationResult:
        ...
        if not raw_hazards:
            raw_text = (vlm_output.get("scene_description", "") + " " + vlm_output.get("raw_model_response", "")).lower()
            
            # Check for critical life-safety hazards or injuries described in text
            if any(k in raw_text for k in ["injur", "pain", "tipped", "overturn", "rubble", "precarious", "trapped", "collapse", "crane", "live wire"]):
                result.hazards.append(
                    DetectedHazard(
                        hazard_type=HazardType.HEAVY_MACHINERY if any(k in raw_text for k in ["machin", "excavat", "bulldozer", "tipped", "crane"]) else HazardType.GENERAL,
                        description=vlm_output.get("scene_description", "Critical hazard / worker injury detected in scene analysis"),
                        location="Scene",
                        severity=SeverityLevel.CRITICAL,
                        worker_in_danger=True,
                    )
                )
                result.overall_severity = SeverityLevel.CRITICAL
                result.escalate = True
                logger.info("CRITICAL hazard inferred from scene text description (no raw_hazards array)")
                return result

            # Check ppe_compliance for violations
